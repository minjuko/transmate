...
export const ScheduleContextProvider = ({children}) => {
  ...
  const onCreate = async ({title, date, time}) => {
    ...
    try {
      const createScheduleUrl = `http://example.invalid:8080/schedule/create/${user.uid}`;
  ...
  const onModify = async modified => {
    ...
    try {
      const modifiedScheduleUrl = `http://example.invalid:8080/schedule/patch/${modified.id}`;
  ...
  const onRemove = async id => {
    ...
    try {
      const deleteScheduleUrl = `http://example.invalid:8080/schedule/delete/${id}`;
