...
export const FileContextProvider = ({children}) => {
  ...
  const onCreate = async ({title, department, content, date}) => {
    ...
    try {
      const setFileUrl = `http://example.invalid:8080/meeting/create/${user.uid}`;
  ...
  const onModify = async modified => {
    ...
    try {
      const modifyFileUrl = `http://example.invalid:8080/meeting/patch/${modified.id}`;
  ...
  const onRemove = async id => {
    ...
    try {
      const deleteUrl = `http://example.invalid:8080/meeting/delete/${id}`;
