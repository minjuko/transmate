...
DB 스키마는 Flyway가 관리한다. 빈 DB에는 전체 초기 스키마를 생성하고, 기존 Flyway 이력이 없는 DB는 버전 1로 baseline 처리한 뒤 이후 마이그레이션만 적용한다. 애플리케이션 시작 전에 DB 백업을 만들고, 동일한 DB 엔진의 검증 환경에서 마이그레이션을 먼저 실행해야 한다. 현재 버전 2 마이그레이션은 더 이상 사용하지 않는 `account.password` 컬럼을 제거한다.
